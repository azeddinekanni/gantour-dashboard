...
if "dashboard_loaded" not in st.session_state:
    with st.spinner("Preparation du tableau de bord..."):
...
with tab_overview:
    ...
    st.markdown("")
    st.markdown("#### Chaine de valeur nationale du phosphate")
    st.caption(
    ...
    sites_data = []
    for code, label in MINE_LABELS.items():
        df_site = last_30[last_30["mines"] == code]
    ...
    legend_items = [
        ...
        ("\U0001F3ED Transformation / Port", "#FFB020"),
        ("\u2693 Port", "#3B82F6"),
    ]
    for col, (txt, color) in zip(legend_cols, legend_items):
        with col:
            st.markdown(f'<div style="color:{color};font-size:12.5px;font-weight:600;">{txt}</div>', unsafe_allow_html=True)
...
with tab_econometrics:
    with st.container(border=True):
        ...
        if len(selected_features) >= 1:
            with st.spinner("Calcul du modele econometrique..."):
...
with tab_predict:
    with st.container(border=True):
        ...
        if len(selected_features_p) >= 1:
            ...
            with st.spinner("Entrainement du modele predictif..."):
